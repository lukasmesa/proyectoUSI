$(function () {
    ...
    var clase = 'software';  // la clase que implementa el CRUD para este grid
    var idPager = 'software-pager';  // la barra de navegación del grid ubicada en la parte inferior

    // las columnas de un grid se definen como un array de objetos con múltiples atributos
    var columnas = [
        {'label': 'Id Software', name: 'id_software', index: 'id_software', width: 100, sortable: true, editable: true, editrules: {required: true, number: false, minValue: 1},
    ...
    // inicializa el grid
    var grid = jQuery('#software-grid').jqGrid({
        ...
        autowidth: false,
        shrinkToFit: false,
        sortname: 'id_software', // <-- OJO pueden ir varias columnas separadas por comas
        sortorder: "asc",
        height: altoGrid,
        width: anchoGrid,
        pager: "#" + idPager,
    ...
    function validarOrdenProduccion(valor, columna) {
        ...
        if (columna == 'id_software') {
            if (valor === '0') {
                return [false, "Falta seleccionar el id del software"];
            }
        }
        if (columna == 'nombre') {
            if (valor === '0') {
                return [false, "Falta seleccionar el nombre del software"];
            }
        }
        if (columna == 'descripcion') {
            if (valor === '0') {
                return [false, "Falta seleccionar la descripcion del software"];
            }
        }
        return [true, ""];
    }
